...
- **Atomic file size limit** — No source file may exceed **1000 lines**. Hard ceiling is **1500 lines** (treat as a build-blocking violation). Files approaching 800 lines should be proactively split. Strategies: extract sub-components, split routes by domain, separate sheets/dialogs from list screens, move helpers into `lib/` or `utils/`. Known violators (re-measured 2026-09-25 with `wc -l` against the real files — several grew since they were first flagged, and the `atlas.finance` files listed in earlier revisions of this note no longer exist, see Phase 8 above): `apps/api/src/index.js` was 6054 lines; decomposed the same day into `routes/company-routes.js`, `routes/contacts-routes.js`, `routes/hr-routes.js`, `routes/identity/*` (5 files: `index.js`, `identity-shared.js`, `identity-roles-routes.js`, `identity-users-routes.js`, `identity-user-detail-routes.js`, `identity-permission-grants-routes.js`) plus shared helpers in `lib/` (`tenant-provisioning.js`, `avatar-url-map.js`, `upload-identity-avatar.js`, `signed-url-by-file-id.js`, `send-password-reset-email.js`, `forgot-password-rate-limit.js`) — `index.js` is now 2306 lines. Still over the 1000-line soft limit; the next extraction candidates left inline are `/profile/me/*`, `/auth/*`, `/setup/initialize`, `/realtime/*`, `/collaboration/invitations*`, and `/runtime/modules`/`/blueprints`. Verified 2026-09-25: real API boot (`node apps/api/src/index.js`) + curl smoke tests against every extracted domain including 401-gated identity/permission-grants routes, plus the full `node --test` suite (1733/1734 pass — the one failure, `storefront-capture-foundation-contract.test.js`, is pre-existing and unrelated to this refactor). `FormFields.jsx` (2633 → 35-line barrel + 5 files ~520 each), `ModuleCatalog.jsx` (1799 → 874 + 3 new components/lib), `calendar-routes.js` (1040 → 632 + `calendar-google-routes.js`), and `pos-routes.js` (806 → 559 + `pos-floor-routes.js`) were also decomposed the same day, each verified with lint + tests + (for the two API files) a real boot; `ModuleCatalog.jsx`/`FormFields.jsx` were verified with `pnpm build:web`. `apps/api/src/routes/chat/index.js` (971) and `chat-service.js` (787) remain under 1000 and are not violators. `HrEmployeeDetail.jsx`/`HrEmployeeForm.jsx` (126/149) were already decomposed earlier and are no longer violators.
...
- Phase 8 (Finance): retired 2026-05-28 — the `atlas.finance` module (`FinanceScreen.jsx`, `finance-service.js`, `finance-documents-service.js`, `finance-posting-service.js`, `finance-aging-service.js`) was deleted outright and replaced by `runly.ledger`; the phase's original scope (double-entry accounting, AR/AP, FX, taxes, aging, applications, reversal) is not carried forward as-is
