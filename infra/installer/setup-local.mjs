...
const transcriberImage =
  (process.env.RUNLY_TRANSCRIBER_IMAGE ?? "raulbellosom/runlyerp:transcriber-latest");
const ttsImage =
...
// Mismo patron que removeInactiveLiveKitServices: si TRANSCRIPTION_MODE pasa
// de "local" a "disabled" en una re-ejecucion, el contenedor no debe seguir
// corriendo. El rol de PostgreSQL de minimo privilegio NO se elimina
// automaticamente aqui (dropTranscriberRole es una accion mas sensible,
// pensada para ejecutarse deliberadamente vía --drop, no como efecto
// colateral silencioso de cambiar una variable de entorno).
function removeInactiveTranscriptionServices(transcriptionMode) {
  if (transcriptionMode === "local") return;
  tryRun("docker", [
    "compose", ...composeFiles,
    "--profile", "transcription-local",
    "rm", "--stop", "--force", "runly-transcriber-local",
  ]);
}

// Same pattern as removeInactiveTranscriptionServices, but a single service
// (no "-local"/"-external" split — runly-tts is stateless and shared by both
// setup scripts, see its comment in docker-compose.yml).
...
async function writeLocalEnv(supabaseInput, identity) {
  ...
  // Transcripcion (docs/TRANSCRIPTION_SPEC.md §6) — enum de solo 2 valores a
  // proposito, sin un tercer valor "api" reservado-sin-implementar (decision
  // explicita del usuario: no exponer configuracion que aparente ofrecer una
  // funcionalidad que todavia no existe).
  const transcriptionMode = fromLocalEnv("TRANSCRIPTION_MODE") || "disabled";
  if (!["local", "disabled"].includes(transcriptionMode)) {
    throw new Error(`TRANSCRIPTION_MODE must be "local" or "disabled" (got "${transcriptionMode}").`);
  }
  const whisperModel = fromLocalEnv("WHISPER_MODEL") || "small";
  const whisperComputeType = fromLocalEnv("WHISPER_COMPUTE_TYPE") || "int8";
  // Sin autodeteccion silenciosa — hallazgo real de la Etapa 1: el numero de
  // hilos que faster-whisper autodetecta depende de los nucleos VISIBLES del
  // contenedor, no del limite real de deploy.resources.limits.cpus, lo que
  // infla RAM/tiempo de forma impredecible entre maquinas distintas.
  const whisperCpuThreads = fromLocalEnv("WHISPER_CPU_THREADS") || "2";
  // Preservado entre re-ejecuciones exactamente igual que las credenciales de
  // LiveKit (apiKey/apiSecret) — generado solo la primera vez que se activa.
  let transcriberDbPassword = fromLocalEnv("TRANSCRIBER_DB_PASSWORD");
  if (transcriptionMode === "local") transcriberDbPassword ||= generateTranscriberPassword();
  const transcriberDatabaseUrl = transcriptionMode === "local" && transcriberDbPassword
    ? buildTranscriberDatabaseUrl(supabase.dbUrl, transcriberDbPassword)
    : "";

  // TTS ("leer en voz alta" en MirAI, docs/TASKS.md) — mismo enum de 2
  // valores que TRANSCRIPTION_MODE y por la misma razon: no exponer una
  // opcion ("cloud"/proveedor pago) que todavia no existe. A diferencia de
  // la transcripcion, no hay credencial de base de datos que generar — el
  // servicio es sin estado, ver docker-compose.yml.
  const miraiTtsMode = fromLocalEnv("MIRAI_TTS_MODE") || "disabled";
  if (!["local", "disabled"].includes(miraiTtsMode)) {
...
# disabled: no se instala ni se activa ningun contenedor ni UI de transcripcion.
TRANSCRIPTION_MODE=${transcriptionMode}
WHISPER_MODEL=${whisperModel}
WHISPER_COMPUTE_TYPE=${whisperComputeType}
WHISPER_CPU_THREADS=${whisperCpuThreads}
TRANSCRIBER_DB_PASSWORD=${transcriberDbPassword}
...
async function main() {
  ...
  if (transcriptionMode === "local") {
    run("docker", [
      ...
      ...migrateNetworkArgs,
      "--env-file", localEnvFile,
      resolvedApiImage, "pnpm", "db:provision-transcriber-role",
    ]);
  }
