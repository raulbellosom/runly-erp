...
- [x] ~~Automatic side-by-side content flow~~ — **REVERTED 2026-09-16** after live user testing: the float-based wrap (`docs/superpowers/specs/2026-09-16-notes-side-by-side-layout-design.md`) made drag-drop targets unclear/invisible, images unpredictably queued into more columns than intended, and paragraph text started mid-line in a confusing way. Images stack full-width again; drag-reorder's drop-position logic is back to its simpler single-column form. If revisited, user prefers an explicit, deliberate layout (Notion-style columns) over automatic CSS floats — not yet spec'd.
- [x] Table drag-reorder (`docs/superpowers/specs/2026-09-16-notes-table-drag-reorder-design.md`): tables can now be reordered on both platforms via a floating overlay handle (not a custom NodeView — TipTap's table extension already claims its own view for column resizing) — mobile reuses the existing "Opciones de tabla" button (press-and-hold now also drags, tap still opens the sheet), desktop gets a new small grip handle at the table's top-left corner. `useImageDragReorder` generalized to `useBlockDragReorder` (`getBoxEl`/`getFrameEl` functions instead of refs) so both images and tables share the same drag mechanics; new `findTableAtSelection` helper in `dragReorder.js`.
  Verified: 2026-09-16 (automated only — `node --test` 115/115 notes pass, `pnpm lint` clean, `vite build` clean). Manual QA at 390px/1440px, both themes, still pending — needs a live browser pass.
