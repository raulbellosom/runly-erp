...
No hecho en esta etapa, decisión deliberada documentada (no un olvido): **V2 (identificación de hablantes por captura de pista de LiveKit, spec §0.2/Etapa 3)** — requiere validar `startTrackEgress` contra un LiveKit real en vivo, algo que no se pudo hacer sin una llamada real activa; escribir esa lógica sin poder probarla contra LiveKit real se consideró peor que no escribirla. **Etapa 5 (integración con MirAI — resumen/tareas/eventos propuestos desde una transcripción)** — el encargo original la marca explícitamente como preparación de arquitectura, no implementación de esta fase; el patrón a seguir (`recognize`/`commit` de la importación de estados de cuenta de `runly.ledger`) ya está identificado y documentado en la spec §7, sin código nuevo.
...
- [x] `apps/api/src/routes/chat/mirai-tts-service.js` (archivo nuevo, no agregado a `mirai-service.js` que ya está en 1001 líneas — por encima del límite blando de 1000): `synthesize(text)` — 503 si `MIRAI_TTS_URL` no está configurado, 400 si el texto está vacío o excede 2000 caracteres, 502 si `runly-tts` no responde o responde con error. Sin permiso nuevo: `chat.mirai.use` (el mismo que ya autoriza ver el texto) también autoriza escucharlo, mismo principio que copiar un mensaje al portapapeles no necesita su propio permiso. Ruta nueva `POST /chat/mirai/tts` + `GET /chat/mirai/status` ahora también reporta `tts: boolean`. 18/18 tests (`mirai-tts-service.test.js` nuevo + `mirai-routes.test.js` extendido)
- [x] SDK: `runly.chat.mirai.speak(text, token)` — usa `requestBlob` (ya existía en `packages/sdk/src/index.js` para exports de Excel/PDF, nunca antes expuesto al dominio `chat`), no `request`, porque la respuesta es audio, no JSON
- [x] UI: botón 🔊 por burbuja de respuesta en `MirAIPanel.jsx` (el panel lateral "solo tú ves esto") vía `useSpeakMirai()` en `useMirAI.js` — un único `<audio>` compartido por instancia del hook, así que un segundo clic (misma burbuja o distinta) detiene lo que ya sonaba en vez de solaparlo; se detiene también al cerrar el panel
...
- No hecho a propósito: botón de lectura en la conversación principal de MirAI (tipo `mirai`, la burbuja de chat normal) — solo en el panel lateral (Spec 2), que es la superficie que el propio PoC imaginó ("función on-demand en MirAI"); extenderlo a la burbuja de chat normal tocaría un componente de mensajes compartido con conversaciones humanas, fuera de alcance de esta ronda
...
- **Pendiente antes de estar realmente disponible**: `pnpm docker:release:tts` real (la imagen `raulbellosom/runlyerp:tts-latest` que el compose referencia todavía no existe en Docker Hub) — documentado, no un bug oculto, mismo estado en el que quedó `runly-transcriber` hasta su primer release real
