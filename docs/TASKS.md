...
- [x] Image drag-reorder UX (`docs/superpowers/specs/2026-09-16-notes-image-drag-reorder-design.md`): replaced the cramped move-handle button (overlapping the top-right corner resize handle's hit area) with press-and-hold-anywhere dragging (long-press on touch, instant on mouse), a floating clone that follows the pointer, and continuous sibling reflow (`useImageDragReorder.js`, `dragReorder.js`'s `computeShiftMap`/`exceedsDragThreshold`).
- [x] Automatic side-by-side content flow (`docs/superpowers/specs/2026-09-16-notes-side-by-side-layout-design.md`): resized images now float left so following text/images/tables/etc. wrap into the leftover space, with a `.tiptap::after` clearfix preventing the trailing-float container-collapse bug, and the drag-reorder drop-position math (`groupIntoRows`/`pickDropIndex`) made row-aware so dropping between two floated images works correctly. Tables never wrap (documented exception — their `overflow-x: auto` scroll wrapper forces a block formatting context).
  Verified: 2026-09-16 (automated only — `node --test` 118/118 notes pass, `pnpm lint` clean, `vite build` clean). Manual QA at 390px/1440px, both themes, still pending — needs a live browser pass.
