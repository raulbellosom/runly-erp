...
Verified: 2026-09-20 (`node --test apps/api/src/routes/ledger/__tests__/*.test.js` → 37 passing; `npx eslint` clean across all touched backend/frontend files; `pnpm build:web` clean, `AiImportScreen` emitted as its own lazy chunk; manual browser upload of the real sample PDF NOT run in this session — needs `GROQ_API_KEY` configured, pending)
