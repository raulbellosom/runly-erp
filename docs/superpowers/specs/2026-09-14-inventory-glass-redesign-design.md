...
5. Riesgo: `RunlyForm`'s campo tipo `relation` no usa `CreatableComboboxField` — usa su propio `RelationSelectField` + un flujo de "crear en línea" que abre un blueprint `FORM` anidado a través de `GET /blueprints` (una tabla de la base de datos exclusiva de módulos RME3). `runly.inventory` es Phase 1-2 y no tiene filas en esa tabla, así que ese flujo de creación en línea **no funciona** para categoría/marca/ubicación dentro del nuevo formulario. Mitigación: los campos `categoryId`/`brandId`/`locationId` del blueprint se configuran como `relation` de solo selección (sin bloque `create`) — el usuario sigue pudiendo elegir cualquier categoría/marca/ubicación existente, pero crear una nueva de esas tres catálogos requiere ir primero a la pantalla "Catálogos" (`InventoryCatalogsScreen.jsx`, fuera de alcance, no se toca). Es una pérdida de conveniencia real frente a la pantalla actual (que sí permite crear al vuelo con `CreatableComboboxField`), documentada aquí a propósito en vez de dejarla pasar en silencio; ver sección 28 para la mejora futura.
6. Riesgo: los nuevos tipos de sección `"component"` (en `RunlyDetail`) y `"custom-fields"` (en `RunlyForm`) son capacidades genuinamente nuevas en renderers que ya usan Fleet y potencialmente otros módulos — no una reutilización de algo existente (corrección hecha en la sección 17 tras leer el código real). Mitigación: ambos tipos son aditivos y solo se activan cuando un blueprint declara explícitamente esa cadena de tipo; ningún blueprint existente (Fleet, HR, etc.) usa esas cadenas hoy, y el comportamiento por defecto de ambos renderers ante un tipo de sección desconocido ya es "no romper, renderizar vacío" — confirmado leyendo `normalizeSections` en ambos archivos antes de escribir el plan de implementación.
...
6. Restaurar la creación rápida de categoría/marca/ubicación sin salir del formulario de alta/edición (perdida en esta versión por la incompatibilidad documentada en el riesgo 5) — por ejemplo agregando a `RunlyForm` una variante ligera de `relation.create` que solo mande `POST {name}` y seleccione el id devuelto, en vez de exigir un blueprint `FORM` anidado vía `/blueprints`.
