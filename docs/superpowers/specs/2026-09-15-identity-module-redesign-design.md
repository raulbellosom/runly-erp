...
7. Roles list view modes are reduced from three (table/card/grid) to two (table/card); the compact icon-grid view is dropped rather than rebuilt as a new blueprint view type.
...
- Roles list: `RunlyCrudView` with table + card view toggle (`ViewModeSwitch` limited to those two modes), search, and the existing "Estado" filter. "Nuevo rol" keeps opening the existing 3-field Sheet (key/nombre/descripción) — not converted to a blueprint form.
...
7. Given the Roles list screen, when the viewer switches between table and card view, then no third (grid) view option is present.
