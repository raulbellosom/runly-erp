...
export function ZoomControl({ zoom, onZoomChange, onReset }) {
  return (
    <div className="absolute bottom-4 right-4 z-30 flex items-center gap-1 rounded-full border border-border bg-card/95 backdrop-blur-sm shadow-sm px-1.5 py-1">
      {onReset && (
        <Button type="button" variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={onReset} title="Restablecer zoom al 100 %" aria-label="Restablecer zoom al 100 %">
          <Scan size={14} />
        </Button>
      )}
      <button
