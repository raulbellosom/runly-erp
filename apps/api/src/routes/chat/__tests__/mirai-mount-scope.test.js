// apps/api/src/routes/chat/__tests__/mirai-mount-scope.test.js
//
// Regression: the MirAI sub-app must NOT install its auth guard at the app
// root. It used to do `mirai.use("*", authMiddleware)` + `app.route("", mirai)`,
...
test("unauthenticated /public/site/* is NOT swallowed by the MirAI auth guard", async () => {
...
test("MirAI routes still require authentication", async () => {
  const res = await buildApp().request("/chat/mirai/status");
