...
import { createExternalInboxRoutes } from "./external-inbox-routes.js";
...
export function createChatRouter({ prisma, supabaseAdmin, authMiddleware, requirePermission, notificationService = null, broadcaster = null, resolveUserContext = null, officeService = null }) {
  ...
  // Channel creation/directory, linked channels, joining, and per-channel
  // roles (Phase A foundation) — see channel-routes.js.
  internal.route("", createChannelRoutes({ requirePermission, chatService, channelDirectoryService, channelLinksService, permissionsService }));
  ...
  // External inbox (operators) — see external-inbox-routes.js
  internal.route("", createExternalInboxRoutes({ requirePermission, chatExternalInboxService, chatService, prisma, broadcaster }));
